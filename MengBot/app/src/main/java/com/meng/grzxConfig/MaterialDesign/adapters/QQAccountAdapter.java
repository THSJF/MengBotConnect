...
public class QQAccountAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        File imageFile = new File(MainActivity2.mainFolder + (isGroup ? "group/" : "user/") + qqNotReply + ".jpg");
        if (imageFile.exists()) {
            holder.imageView.setImageBitmap(BitmapFactory.decodeFile(imageFile.getAbsolutePath()));
        } else {
			MainActivity2.instance.threadPool.execute(new DownloadImageRunnable(context, holder.imageView, qqNotReply, isGroup ? 0 : 1));
        }
